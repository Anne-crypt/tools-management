<template>
    <h2 class="mb-4 text-lg font-semibold text-slate-900 dark:text-white">Activité par département</h2>
...
const props = defineProps<{
  tools: Array<{ ownerDepartment: string; activeUsersCount: number }>;
}>();

const chartData = computed(() => {
  // 1. Agrégation des données par département
  const statsByDept = props.tools.reduce((acc, tool) => {
