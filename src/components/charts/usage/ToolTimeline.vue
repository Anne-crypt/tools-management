<template>
  <section class="rounded-2xl border border-slate-100 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-black">
    <div class="mb-6">
    ...
    <div class="relative space-y-6 border-l border-slate-200 pl-6 dark:border-slate-800">
      <article
        v-for="(tool, index) in sortedTools"
        :key="tool.id"
        class="relative"
      >
        <div class="absolute -left-8 top-1 flex h-4 w-4 items-center justify-center rounded-full border-2 border-white bg-blue-500 shadow-sm dark:border-black">
          <span class="h-2 w-2 rounded-full bg-white"></span>
        </div>

        <div class="pl-0 md:pl-0">
          <div class="mb-2 text-xs font-medium text-slate-400 dark:text-slate-500 md:absolute md:-left-32 md:top-4 md:w-24 md:text-right">
            <div>#{{ index + 1 }}</div>
