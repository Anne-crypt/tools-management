<template>
  <h2 class="mb-4 text-lg font-semibold text-slate-900 dark:text-white">
    Outils les plus et les moins utilisés
  </h2>
...
const chartData1 = computed(() => {
  const sortedTools = [...props.tools]
    .filter((tool) => Number.isFinite(tool.activeUsersCount ?? 0))
    .sort((a, b) => (b.activeUsersCount ?? 0) - (a.activeUsersCount ?? 0))
    .slice(0, 10);

  const usersData = sortedTools.map((tool) => tool.activeUsersCount ?? 0);
...
const chartData2 = computed(() => {
  const sortedTools = [...props.tools]
    .filter((tool) => Number.isFinite(tool.activeUsersCount ?? 0))
    .sort((a, b) => (a.activeUsersCount ?? 0) - (b.activeUsersCount ?? 0))
    .slice(0, 10)
    .reverse();
